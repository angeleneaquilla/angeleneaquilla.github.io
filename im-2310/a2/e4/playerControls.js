...
rewindButton.addEventListener("click", () => {
  ...
  /*I set the rewind and fast forward by 10 seconds because it is a sufficient amount
  of time given the duration of the video*/
});
...
loopButton.addEventListener("click", () => {
  ...
  if (videoElement.loop) {
    loopButton.style.backgroundColor= "#743160";
    /*The loop button needed an indicator that it is selected so the user knows the video
    is going to be looped/unlooped so I changed it to the same color as the background of 
    the website*/
  } else {
...
function muteUnmute(){
  if (videoElement.muted) {
    ...
    /*Similarly to the pause and unpause button, I needed to add an event listener to the 
    mute/unmute button that toggles the video element's "muted" property on and off.   */
  }
}
